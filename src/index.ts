      ...
      body: [
        {
          ...
          source: {
            type: 'Literal', value: '@omurilo/remark-mermaid/Mermaid'
          },
        },
      ],
    },
...
function stringifyObject(object: string | Object): string {
  ...
  if (Array.isArray(object)) return `[${object.map(stringifyObject).join(', ')}]`;
  return Object.entries(object).map(([k, v]) => ` { ${k}: ${stringifyObject(v)} }`).join().trim()
}

function parseObjectToJsxAttribute(object: any): Record<string, unknown> | Array<Record<string, unknown>> {
...
export const remarkMermaid: Plugin<[PluginOptions], Root> = (options: PluginOptions) => (ast, _file, done) => {
  ...
  if (codeblocks.length !== 0) {
    ...
    const classNameOption = generateJsxAttribute('className', options.className);
